...
import { useChatSessions, useChatMessages } from '../hooks/usePsychologists';
...
const PsychologistChat: React.FC = () => {
  ...
  const { sessions, loading: sessionsLoading, markAsRead } = useChatSessions();
  const { messages, loading: messagesLoading } = useChatMessages(selectedSession);

  // Filtrar sesiones donde el usuario actual es el psicólogo
  const mySessions = sessions.filter(session => session.psychologistId === user?.uid);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // await sendMessage(selectedSession, newMessage);
      setNewMessage('');
    } catch (error) {
