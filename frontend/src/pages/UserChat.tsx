...
const UserChat: React.FC = () => {
  ...
  const [showPsychologistSelector, setShowPsychologistSelector] = useState(false);
  ...
  const startChatWithPsychologist = async (psychologistId: string) => {
    ...
    try {
      ...
      const existingSession = mySessions.find(session => session.psychologistId === psychologistId);
      
      if (existingSession) {
        // Si ya existe, usar esa sesión
        setSelectedSession(existingSession.id);
        setShowChatOnMobile(true);
        toast.success('Conversación existente abierta');
        setShowPsychologistSelector(false);
        return;
      }
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div
                  ...
                  <button
                    onClick={() => {
                      if (availablePsychs.length > 0 && !isCreatingChat) {
                        setShowPsychologistSelector(true);
      ...
      {/* Modal de Selección de Psicólogos */}
      {showPsychologistSelector && (
        <div className='fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4'>
          <div
            className={`rounded-xl shadow-lg max-w-2xl w-full max-h-[80vh] overflow-hidden transition-colors duration-500 ${
              isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
            } border`}
          >
            {/* Header del Modal */}
            ...
            >
              <div className='flex items-center justify-between'>
                ...
                <button
                  onClick={() => setShowPsychologistSelector(false)}
                  className={`p-2 rounded-lg transition-colors duration-300 ${
                    isDarkMode
              ...
              >
                Elige un psicólogo para iniciar una conversación. Si ya tienes una conversación con él, se abrirá automáticamente.
              </p>
            </div>
            ...
            <div className='overflow-y-auto max-h-[60vh] p-4'>
              {psychologistsLoading ? (
                ...
                <div className='space-y-3'>
                  {availablePsychs.map((psychologist) => {
                    const hasExistingSession = mySessions.some(
                      session => session.psychologistId === psychologist.userId
                    );
                        ...
                        <div className='flex items-center justify-between'>
                          <div className='flex items-center space-x-3 flex-1'>
                            ...
                            <div className='flex-1 min-w-0'>
                              ...
                              >
                                {psychologist.displayName || psychologist.name}
                              </h3>
                              <p
                                className={`text-sm transition-colors duration-500 ${
                                  isDarkMode ? 'text-gray-400' : 'text-gray-600'
                              ...
                              >
                                {psychologist.specialization || 'Psicólogo'}
                              </p>
                              {hasExistingSession && (
                                <p
                                  className={`text-xs mt-1 transition-colors duration-500 ${
                                ...
                                >
                                  Ya tienes una conversación con este psicólogo
                                </p>
                              )}
                            </div>
                          </div>
