...
const MoodHistoryCard: React.FC<MoodHistoryCardProps> = ({ moodLog, isDarkMode = false, onEdit }) => {
  ...
  const getMoodBackground = (mood: number) => {
    if (mood >= 9) return isDarkMode ? 'bg-green-900/50 border border-green-700' : 'bg-green-50 border border-green-200';
    if (mood >= 7) return isDarkMode ? 'bg-blue-900/50 border border-blue-700' : 'bg-blue-50 border border-blue-200';
    if (mood >= 5) return isDarkMode ? 'bg-yellow-900/50 border border-yellow-700' : 'bg-yellow-50 border border-yellow-200';
    if (mood >= 3) return isDarkMode ? 'bg-orange-900/50 border border-orange-700' : 'bg-orange-50 border border-orange-200';
    return isDarkMode ? 'bg-red-900/50 border border-red-700' : 'bg-red-50 border border-red-200';
  };
  ...
  const formatDate = (date: Date) => {
    try {
      ...
      // Manejar diferentes tipos de fecha (Firebase Timestamp, Date, string)
